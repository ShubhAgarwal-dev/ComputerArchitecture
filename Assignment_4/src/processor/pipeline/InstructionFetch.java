...
public class InstructionFetch {
	...
	public void performIF() {
		if (IF_EnableLatch.isIF_enable()) {
			if(containingProcessor.getDataLockUnit().getStallDuration()> Clock.getCurrentTime()) {
				if (containingProcessor.isBranchTaken()) {
					...
					// System.out.println("[Debug] (IF) Branch taken, PC updated to " +
					// containingProcessor.getBranchPC());

				} else {
					containingProcessor.getRegisterFile()
							.setProgramCounter(containingProcessor.getRegisterFile().getProgramCounter() + 1);
					// System.out.println("[Debug] (IF) PC incremented to " +
